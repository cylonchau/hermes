...
func TestRecordDAO_Mock_ListSRVRecords(t *testing.T) {
	db, mock, err := setupMockDB()
	assert.NoError(t, err)
	dao := NewRecordDAO(db)
	ctx := context.Background()
	...
	recordRows := sqlmock.NewRows([]string{"id", "name", "type", "zone_id", "view_id"}).AddRow(10, "www", "SRV", 5, 1)
	...
	mock.ExpectQuery("^SELECT .*? FROM `record_srv` JOIN record ON record.id = record_srv.record_id WHERE record.view_id = \\?$").
		WithArgs(viewID).
	...
	res, err := dao.ListSRVRecords(ctx, &viewID)
