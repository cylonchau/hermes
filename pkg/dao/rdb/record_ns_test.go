...
func TestRecordDAO_Mock_ListNSRecords(t *testing.T) {
	db, mock, err := setupMockDB()
	assert.NoError(t, err)
	dao := NewRecordDAO(db)
	ctx := context.Background()
	...
	recordRows := sqlmock.NewRows([]string{"id", "name", "type", "zone_id", "view_id"}).AddRow(10, "www", "NS", 5, 1)
	...
	mock.ExpectQuery("^SELECT .*? FROM `record_ns` JOIN record ON record.id = record_ns.record_id WHERE record.view_id = \\?$").
		WithArgs(viewID).
	...
	res, err := dao.ListNSRecords(ctx, &viewID)
