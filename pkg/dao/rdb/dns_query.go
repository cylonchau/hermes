...
func (dao *RecordDAO) QuerySOARecord(ctx context.Context, zoneName string) (*model.SOARecord, error) {
	...
	err := dao.db.WithContext(ctx).
		...
		Where("`zone`.name = ? AND `record`.name IN (?, '@') AND `zone`.is_active = 1 AND `record`.is_active = 1",
			zoneName, zoneName).
		Order("`record_soa`.id ASC").
		Scan(&soaRecord).Error
	if err != nil {
		return nil, err
	}
